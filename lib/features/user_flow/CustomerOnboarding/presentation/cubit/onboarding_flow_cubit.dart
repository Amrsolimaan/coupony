...
import 'package:coupony/core/constants/category_constants.dart';
...
class OnboardingFlowCubit extends Cubit<OnboardingFlowState> {
  ...
  Future<void> submitOnboarding() async {
    if (!state.isStep1Valid) {
      _safeEmit(
        state.copyWith(errorMessageKey: 'error_onboarding_step1_incomplete'),
      );
      return;
    }
    if (!state.isStep2Valid) {
      _safeEmit(
        state.copyWith(errorMessageKey: 'error_onboarding_step2_incomplete'),
      );
      return;
    }
    ...
    _safeEmit(
      state.copyWith(isSaving: true, errorMessageKey: null, apiErrorKey: null),
    );
  ...
  Future<void> _initializeInterestScores() async {
    final result = await initInterestScoresUseCase(state.selectedCategories);

    result.fold(
      (failure) {
        logger.e(
          'Failed to initialize interest scores: ${failure?.message ?? ''}',
        );
      },
      (_) {
        logger.i(
