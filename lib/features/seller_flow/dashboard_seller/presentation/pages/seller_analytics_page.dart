...
class SellerAnalyticsPage extends StatelessWidget {
    ...
    return BlocConsumer<SellerAnalyticsCubit, SellerAnalyticsState>(
      ...
      builder: (context, state) {
        return Scaffold(
          ...
          appBar: (state is SellerAnalyticsGuest || state is SellerAnalyticsPending)
  ...
  Widget _buildBody(
    ...
    // ── Guest View ─────────────────────────────────────────────────────────
    if (state is SellerAnalyticsGuest) {
    ...
    if (state is SellerAnalyticsPending) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // ── Title ──────────────────────────────────────────────────────
            Text(
              l10n.seller_analytics_content_title,
              textAlign: TextAlign.center,
              style: AppTextStyles.customStyle(
                context,
                fontSize: 20,
            ...
            // ── Subtitle ───────────────────────────────────────────────────
            Text(
              l10n.seller_analytics_content_subtitle,
              textAlign: TextAlign.center,
              style: AppTextStyles.customStyle(
                context,
                fontSize: 14,
  ...
  void _handleNavigation(
    ...
    if (state is SellerAnalyticsInitial) {
      isGuest = state.isGuest;
      isPending = state.isPending;
    } else if (state is SellerAnalyticsLoaded) {
      isGuest = state.isGuest;
      isPending = state.isPending;
    } else if (state is SellerAnalyticsGuest) {
      isGuest = true;
    } else if (state is SellerAnalyticsPending) {
      isPending = true;
    }
